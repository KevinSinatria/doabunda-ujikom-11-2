...
@section('content')
    <section class="h-screen flex justify-center items-center lg:px-20 w-full">
        <form action="{{ route('general.auth.signin') }}" method="post"
            ...
            {{-- Footer of form --}}
            <div class="flex flex-col gap-4 mt-6">
                <p class="text-sm text-end">Belum punya akun? <a href="{{ route('general.auth.signup') }}" class="text-[#ef9f00] hover:text-[#4d451c] transition-all duration-300">Sign Up</a></p>
                <button type="submit"
                    class="w-full px-4 py-2 rounded-lg bg-[#FFAB00] text-white hover:bg-[#FFC107] transition-all duration-300">Sign In
        </form>
    </section>
@endsection
